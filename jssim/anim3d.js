...
function solveContact3(dt) {
  ...
  // soft floors: glide out of the mattress instead of slapping to the surface
  if (her3.root.position.y < 0.13)
    her3.root.position.y = dampNum3(her3.root.position.y, 0.13, 8, step);
  if (him3.root.position.y < 0.16)
    him3.root.position.y = dampNum3(him3.root.position.y, 0.16, 8, step);
}
...
function updateAnim3(dt) {
  ...
  if (rubActive) {
    ...
    for (const [arm, tv, key] of targets) {
      ...
      aimArmAt3(arm, _aimSm3[key], him3.R.upperArm, him3.R.foreArm, -0.12, 10 + 50 * ramp, dt, 10 + 15 * ramp);
    }
  }
